...
- Upgraded claude-okf-repo-kit 0.3.8 → 0.3.10 (0.3.9 audit fixes plus 0.3.10 parity gates; both releases came from this repo's own upgrade friction, and kit ADR 0026 — the trigger-gated Python rewrite boundary — was accepted upstream the same day). The updater refreshed `scripts/okf` and both hooks by provenance and synced the declared `.codex/hooks` mirrors byte-identical in the same run — the `mirrors:` declaration working end to end, no manual re-sync. The refreshed hooks and helper now treat `.agents/` and `Codex.local.md` as agent config, so this repo's `.agents/**` map entries are inert for `check-stale` from here on (they stay mapped as documentation; the ADR-0012 repo-health parity test remains the enforced guard for skill pairing). Lone `CLAUDE.2.md` candidate verified byte-equal to the owner-rendered kit template (no unmerged kit content) and deleted. No spec or ADR change warranted: kit-managed maintenance under existing decisions. Verified: `make check` (68 tests, mirrors byte-identical via the parity guard), `bash scripts/okf check-stale` clean.
